...
async function generateResumePdf({ resume, jobDescription, selfDescription }) {
  ...
  const prompt = `Generate html for the resume for the candidate with the following details:
    ...
    Self Description: ${selfDescription}
      ...
      The format of the response should be a JSON object with a single field "html" which contains the HTML content of the resume. The  resume should be tailored to the job description and should highlight the candidate's strengths and relevant experience. The resume should not sound like it was generated by AI, but should sound natural and human-like. The resume should be concise and should not exceed one A4 page when converted to PDF.There should be no padding in the html content. You can highlight the content using some colors but make sure the overall look of the resume is professional and suitable for job applications. The content should be ATS friendly, meaning it should be easily parsable by Applicant Tracking Systems, and should not include any complex formatting that might confuse the ATS. Try to use bullet points, short sentences, and clear section headings to make the resume easy to read and ATS friendly. Prefer single column layout over multi-column layout for better readability and ATS compatibility.
    `;
